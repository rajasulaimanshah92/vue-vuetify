<template lang="">
    <v-container grid-list-lg>
        <v-layout row wrap>
            <v-flex xs6>
                <h4>Emergency Archive: {{ facilityName }}</h4>
            </v-flex>
            <v-spacer></v-spacer>
                        ...
                        <div id="emergency-list">
                            <v-container fluid grid-list-xl class="pt-0 px-2">
                                ...
                                >
                                    ...
                                    <template v-slot:no-data v-if="loadEmergencyArchive">
...
export default {
    ...
    methods: {
        ...
        getListApi() {
            this.loadEmergencyArchive = true
            axios
                .get(`${process.env.VUE_APP_API_URL}/emergency/emergency/archived/${this.id}`, {
                    headers: {
                        Authorization: 'Bearer ' + this.$store.getters.user.token,
                    },
                })
                .then((response) => {
                    this.loadEmergencyArchive = false
                    let tempArray = []
