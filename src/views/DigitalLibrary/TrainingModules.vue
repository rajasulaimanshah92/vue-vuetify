        ...
        <v-layout row wrap>
            <v-flex xs12>
                <v-card class="pa-3">
                    ...
                    <v-card-actions class="px-6">
                        <h2 class="primaryGreen--text">Training modules</h2>
                        <v-spacer></v-spacer>
                        <v-btn
                    ...
                    <v-container class="pb-0" grid-list-md>
                        <v-layout row wrap>
                            <v-flex xs12 px-2>
                                <v-file-input
                                    accept=".xlsx"
                                    v-model="editedTrainingModules.files"
                                    label="File input"
                                ></v-file-input>
...
export default {
    ...
    methods: {
        ...
        openEditDialog(item) {
            ...
            // this.editedTrainingModules = Object.assign({}, item)
        ...
        saveEditDocument(item) {
            let payload = this.editedTrainingModules.files
            let id = this.editedTrainingModules.id
            axios
                .post(`${process.env.VUE_APP_API_URL}/digitallibrary/edit/${id}`, payload, {
                    headers: {
                        Authorization: 'Bearer ' + this.$store.getters.user.token,
                    },
                })
